...
GRBModel criarModelo(
    ...
    for (size_t i = 0; i < arestas.size(); i++) {
        ...
        t.push_back(std::vector<GRBVar>());
        for (int k = 0; k < DIMENSAO; k++) {
            t[i].push_back(model.addVar(-GRB_INFINITY, GRB_INFINITY, 0, GRB_CONTINUOUS, "t_" + std::to_string(i) + ',' + std::to_string(k + 1)));
        }
    }
    ...
    for (size_t i = 0; i < arestas.size() - 1; i++) {
        ...
        for (int k = 0; k < DIMENSAO; k++) {
            model.addConstr(t[i][k] == x[x_ind][k] - coordenada[k]);
            expr += t[i][k] * t[i][k];
        }
    ...
    GRBQuadExpr expr = 0;
    for (int k = 0; k < DIMENSAO; k++) {
        model.addConstr(t[4][k] == x[0][k] - x[1][k]);
        expr += t[4][k] * t[4][k];
    }
